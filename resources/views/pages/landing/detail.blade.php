...
<div>
    ...
    <nav class="mx-8 mt-8 text-sm lg:mx-20" aria-label="Breadcrumb">
        <ol class="inline-flex p-0 list-none">
            ...
            <li class="flex items-center">
                <p class="font-medium">Website Developer</p>
            </li>
        </ol>
    </nav>
                ...
                <div class="flex overflow-x-scroll hide-scroll-bar dragscroll">
                    <div class="flex mt-2 flex-nowrap">
                        ...
                        <img :class="{'border-4 border-serv-button': active === {{ $item->id }} }"
                            @click="changeThumbnail('{{ Storage::url($item->thumbnail) }}',{{ $item->id }})"
                            src="{{ Storage::url($item->thumbnail) }}" alt=""
            ...
            <div class="content">
                <div x-data="{ tab: window.location.hash ? window.location.hash.substring(1) : 'description' }"
                    ...
                    <div x-show.transition.duration.500ms="tab === 'seller'" class="leading-8 text-md">
                        ...
                        <div class="grid md:grid-cols-12">
                            <div class="flex items-center col-span-12 p-2 lg:col-span-6">
                                <div class="flex items-center space-x-4">
                                    ...
                                    <img src="{{ Storage::url($service->user->user_detail->photo) }}" alt="My profile"
                                        class="w-20 h-20 rounded-full object-cover object-center">
                                    @else
                                    <svg class="w-20 h-20 object-cover object-center rounded-full text-gray-300"
                        ...
                        <div class="mt-4 mb-8 content-description">
                            <p>
                                {{ $service->user->user_detail->biography ?? '' }}
                            </p>
                        </div>
                        <h3 class="my-4 text-lg font-semibold">My Experiences</h3>
                        <ul class="mb-8 list-check">
                            @forelse ($service->user->user_detail->user_experiences as $item)
                        ...
                        <div class="mb-8 skills">
                            ...
                            <span class="inline-block px-3 py-1 mr-2 rounded bg-serv-services-bg">
                                {{ $item->tagline ?? '' }}
                            </span>
                            @empty
                            @endforelse
                        ...
                        <p class="my-4 text-lg text-gray-400">
                            Joined Since {{ $service->user->created_at->format('j F Y') }}
                        </p>
                    </div>
            ...
            <div class="mb-4 border rounded-lg border-serv-testimonial-border">
                ...
                <div class="flex items-start px-4 pt-6">
                    ...
                    <img class="object-cover w-16 h-16 mr-4 rounded-full"
                        src="{{ Storage::url($service->user->user_detail->photo) }}" alt="avatar">
                    @else
                    <svg class="w-16 h-16 object-cover object-center rounded-full mr-4 text-gray-300"
                        fill="currentColor" viewBox="0 0 24 24">
                        <path
                            d="M24 20.993V24H0v-2.996A14.977 14.977 0 0112.004 15c4.904 0 9.26 2.354 11.996 5.993zM16.002 8.999a4 4 0 11-8 0 4 4 0 018 0z" />
                    </svg>
                ...
                <div class="flex items-center px-2 py-3 mx-4 mt-4 border rounded-full border-serv-testimonial-border">
                    <div class="flex-1 text-sm font-medium text-center">
                        ...
                        {{ $service->delivery_time ?? '0' }} Days Delivery
                    </div>
                    <div class="flex-1 text-sm font-medium text-center">
                        <svg class="inline" width="24" height="24" viewBox="0 0 24 24" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                        ...
                        {{ $service->revision_limit ?? '0' }} Revision Limit
                    </div>
                </div>
                ...
                <div class="px-4 pb-4 booking">
                    ...
                    @guest
                    <a href="javascript:void(0)" onclick="toggleModal('loginModal')"
                        class="block px-12 py-4 my-2 text-lg font-semibold text-center text-white bg-serv-button rounded-xl">
...
<script>
    function gallery() {
        return {
            featured: '{{ Storage::url($service->service_thumbnails->first()->thumbnail) }}',
            active: '{{ $service->service_thumbnails->first()->id }}',
